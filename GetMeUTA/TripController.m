...
- (void)findCompleteTripWithStartStation:(NSString *)startStopID endStation:(NSString *)endStopID {
...
//    pulling the stop sequence for the start and end station
    NSString *startStopSequence = [stopIDArray[0] valueForKey:stopSequenceKey];
    NSString *endStopSequence = [stopIDArray[1] valueForKey:stopSequenceKey];
...
//    removing the trips with stop sequence before the start stop sequence and after the end stop sequence
    ...
    for (NSDictionary *dictionary in self.selectedTripInfo) {
        NSString *stopSequenceValue = [dictionary valueForKey:stopSequenceKey];
